...
const rules = {
  ...
  price: {
    required: true,
    validator: (rules: any, val: string, callback: Function) => {
...
const init = async () => {
  querySetmealById(route.query.id).then(res => {
    if (res && res.data && res.data.code === 1) {
      ...
      // imageUrl.value = `http://172.17.2.120:8080/common/download?name=${res.data.data.image}`
...
const submitForm = (formName: any, st: any) => {
  ruleFormRef.value?.validate((valid: any) => {
    if (valid) {
      ...
      prams.status =
        actionType.value === 'add' ? 0 : ruleForm.status ? 1 : 0
